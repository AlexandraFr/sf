...
class Cell:
    ...
    def get_neighbours(self, cell_list, x, y):
        ...
        for neighbour in neighbours:
            p, l = neighbour
            if 0 <= p < len(cell_list) and 0 <= l < len(cell_list[p]):
                if cell_list[p][l].is_alive():
                    count_alive += 1
        self.neigh = count_alive
...
class CellList:
    def __init__(self, cols, rows, randomize=True, p='game.txt'):
        self.cols = cols
        self.rows = rows
        self.cell_list = []
        if randomize:
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                if self.cell_list[i][j].is_alive() == 1:
                    ...
                    pygame.draw.rect(screen, pygame.Color('white'), (j * size, i * size, size, size))
    ...
    def update(self):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                if self.cell_list[i][j].is_alive() == 1:
                    if 2 <= self.cell_list[i][j].neigh <= 3:
                        new_cell_matrix[i][j] = Cell(1)
                    else:
                        new_cell_matrix[i][j] = Cell(0)
                else:
                    if self.cell_list[i][j].neigh == 3:
                        new_cell_matrix[i][j] = Cell(1)
                    else:
                        new_cell_matrix[i][j] = Cell(0)
