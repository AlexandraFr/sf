...
def transposing(grid):  # функция транспонирует паззл, то есть строки становятся столбцами и наоборот
    ...
    # zip составит картеж из всех 1ых, 2ых, 3их и т.д. элементов списков в grid
    # map исп-ся для каждого картежа, полученного с помощью zip и преобразует в список; вернёт картеж
    # полученный картеж нужно преобрзовать в список
